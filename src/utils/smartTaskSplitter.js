 ...
 * 
 * מחלק משימות ארוכות למשימות קצרות של מקסימום 45 דקות
 * מתחשב בדחיפות (רבע אייזנהאואר), זמן פנוי, ותאריך יעד
 * 
 * עדכון: חלוקה ב-45 דקות, לפי דחיפות, ניצול זמן פנוי
 */

// קונפיגורציה
const CONFIG = {
  MAX_TASK_DURATION: 45,        // מקסימום דקות למשימה אחת - עודכן ל-45
  ...
  TYPE_PREFERENCES: {
    ...
    email: { 
      maxDuration: 45,          // מיילים - מקסימום 45 דקות
      preferredHours: [8, 9, 15, 16],
      requiresFocus: false
    },
    client_communication: { 
      maxDuration: 45,          // תקשורת לקוחות
      preferredHours: [10, 11, 14, 15],
      requiresFocus: false
    },
    course: { 
      maxDuration: 45,          // קורס - 45 דקות
...
/**
 * פיצול משימה ארוכה למשימות קטנות של 45 דקות
...
export function splitTask(task, options = {}) {
  ...
  // קביעת אורך מקסימלי למשימה - 45 דקות
  ...
  // חישוב מספר החלקים (כל אחד עד 45 דקות)
  ...
  let minutesUsedToday = 0;
    const maxPartsPerDay = workPreferences.maxTasksPerDay || 6; // פחות חלקים ביום כי כל חלק ארוך יותר
...
export function calculateOptimalSplit(task, existingTasks = [], options = {}) {
  ...
  for (const day of sortedDays) {
    ...
    // כמה זמן נשבץ ביום הזה - עד 45 דקות לחלק
...
export function getSplitRecommendation(task, existingTasks = []) {
  ...
  const taskType = task.task_type || 'other';
  const quadrant = task.quadrant || 2;
  const maxDuration = CONFIG.MAX_TASK_DURATION; // 45 דקות
  
  // אם המשימה קצרה מספיק
  if (duration <= maxDuration) {
    return {
      shouldSplit: false,
      reason: 'המשימה קצרה מספיק (עד 45 דקות) ולא צריכה פיצול'
    };
  }
