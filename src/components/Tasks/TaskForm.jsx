...
import { calculateOptimalSplit } from '../../utils/smartTaskSplitter';
...
function TaskForm({ task, defaultQuadrant = 1, defaultDate = null, defaultTime = null, onClose }) {
  ...
  const splitPreview = useMemo(() => {
    ...
    if (!duration || duration <= 0 || !formData.title) return null;

    const recommendation = getSplitRecommendation(
      {
        title: formData.title,
        estimated_duration: duration,
        task_type: formData.taskType,
        quadrant: formData.quadrant || 2,
  ...
  const splitSchedulePreview = useMemo(() => {
    const duration = parseInt(formData.estimatedDuration, 10);
    if (!duration || duration <= 45 || !formData.title) return null;
    ...
    const result = calculateOptimalSplit(
      {
        ...
        estimated_duration: duration,
        task_type: formData.taskType || 'other',
        quadrant: formData.quadrant || 2,
        start_date: formData.startDate || null,
        due_date: formData.dueDate || null
      },
    ...
    if (!result?.success || !Array.isArray(result.splitTasks)) {
      return {
        success: false,
        message: result?.message || 'לא ניתן לחשב תצוגה מקדימה כרגע'
      };
    }

    const byDate = result.splitTasks.reduce((acc, part) => {
      const key = part.due_date || 'ללא תאריך';
      if (!acc[key]) {
        acc[key] = { count: 0, totalMinutes: 0 };
      }
      acc[key].count += 1;
      acc[key].totalMinutes += part.estimated_duration || 0;
      return acc;
    }, {});
    ...
    const days = Object.entries(byDate)
      ...
      .map(([date, info]) => {
        const existingForDate = (tasks || []).filter(t =>
          !t.is_completed &&
          (t.due_date === date || t.scheduled_date === date)
        );
        const existingMinutes = existingForDate.reduce(
          (sum, t) => sum + (t.estimated_duration || t.estimatedDuration || 30),
          0
        );
        const isOverloaded = (existingMinutes + info.totalMinutes) > (8 * 60);
        const hasUrgent = existingForDate.some(
          t => t.priority === 'urgent' || t.quadrant === 1
        );
    ...
    const overloadedDays = days.filter(d => d.isOverloaded).length;
    const urgentDays = days.filter(d => d.hasUrgent).length;
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
        ...
        {splitPreview && (
          <div className={`mt-2 text-xs p-3 rounded-lg border ${
            splitPreview.recommendation?.shouldSplit
            ...
            {splitPreview.recommendation?.shouldSplit ? (
              <>
                <div className="font-semibold mb-1">
                  ✂️ המשימה תתפצל אוטומטית לבלוקים של עד 45 דקות
                </div>
                <div>
                  צפויים כ-{splitPreview.recommendation.recommendation?.numParts || Math.ceil(splitPreview.duration / 45)} חלקים.
                </div>
                {splitPreview.recommendation?.recommendation?.warning && (
                  <div className="mt-1">
                    ⚠️ {splitPreview.recommendation.recommendation.warning}
                  </div>
                )}
              </>
            ) : (
              <div className="font-semibold">
                ✅ המשימה קצרה (עד 45 דקות) ולא תתפצל.
              </div>
            )}
          </div>
        )}
        {splitSchedulePreview && splitPreview?.recommendation?.shouldSplit && (
          <div className="mt-2 text-xs p-3 rounded-lg border bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800 text-indigo-800 dark:text-indigo-200">
            <div className="font-semibold mb-1">📅 תצוגת שיבוץ צפויה</div>
            {splitSchedulePreview.success ? (
              <>
                <div className="mb-2">
                  {splitSchedulePreview.summary?.totalParts
                    ? `סה"כ ${splitSchedulePreview.summary.totalParts} חלקים`
                    : 'שיבוץ חלקים לפי ימים פנויים'}
                </div>
                <div className="space-y-1">
                  {splitSchedulePreview.days.slice(0, 6).map(day => (
                    <div
                      key={day.date}
                      className={day.isOverloaded ? 'text-red-700 dark:text-red-300 font-medium' : ''}
                    >
                      • {day.date}: {day.count} חלקים ({day.totalMinutes} דק׳)
                      {day.hasUrgent ? ' • דחוף' : ''}
                      {day.isOverloaded ? ' • יום עמוס' : ''}
                    </div>
                  ))}
                  {splitSchedulePreview.days.length > 6 && (
                    <div>• ... ועוד {splitSchedulePreview.days.length - 6} ימים</div>
                  )}
                </div>
                <div className="mt-2 border-t border-indigo-200 dark:border-indigo-700 pt-2 space-y-1">
                  {splitSchedulePreview.overloadedDays > 0 && (
                    <div className="text-red-700 dark:text-red-300">
                      ⚠️ {splitSchedulePreview.overloadedDays} ימים צפויים להיות עמוסים.
                    </div>
                  )}
                  {splitSchedulePreview.urgentDays > 0 && (
                    <div>
                      🔥 קיימים {splitSchedulePreview.urgentDays} ימים עם משימות דחופות.
                    </div>
                  )}
                  {(splitSchedulePreview.overloadedDays > 0 || splitSchedulePreview.urgentDays > 0) && (
                    <div className="font-medium">
                      המלצה: להגדיר תאריך יעד רחב יותר או להפחית משך חלקים ביום עמוס.
                    </div>
                  )}
                </div>
              </>
            ) : (
              <div>⚠️ {splitSchedulePreview.message}</div>
            )}
          </div>
        )}
        {aiPrediction && (
