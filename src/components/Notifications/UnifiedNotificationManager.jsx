...
export function useUnifiedNotifications() {
  ...
  const { user } = useAuth();
  const { notifications: appNotifications } = useSettings();
  ...
  const checkAndNotify = useCallback(() => {
    ...
    // ========================================
    // 6. סיום יום עבודה (הגדרות → התראות → סיום יום — לא היה מחובר ללוגיקה)
    // ========================================
    if (
      appNotifications?.enabled !== false &&
      appNotifications?.endOfDay?.enabled !== false &&
      user?.id &&
      isWorkDay
    ) {
      const timeStr = appNotifications?.endOfDay?.time || '16:00';
      const parts = String(timeStr).split(':');
      const endH = parseInt(parts[0], 10);
      const endM = parseInt(parts[1], 10);
      const endDayMinutesCfg = (Number.isFinite(endH) ? endH : 16) * 60 + (Number.isFinite(endM) ? endM : 0);
      // חלון 15 דק׳ מהשעה המוגדרת — כדי לא לאבד התראה אם הדף נטען מאוחר מעט
      if (currentMinutes >= endDayMinutesCfg && currentMinutes < endDayMinutesCfg + 15) {
        try {
          const doneKey = `zmanit_eod_alert_${user.id}_${today}`;
          if (!localStorage.getItem(doneKey)) {
            localStorage.setItem(doneKey, String(Date.now()));
            logNotificationToHistory('end_of_day', 'סיום יום עבודה', timeStr);
            sendNotification('🌙 סיום יום עבודה', {
              body: `הגיעה השעה ${parts[0]?.padStart(2, '0')}:${(parts[1] || '00').padStart(2, '0')} — זמן טוב לסגור את יום העבודה.`,
              tag: 'end-work-day',
              soundType: 'warning'
            });
            toast.success('🌙 סיום יום עבודה — זמן לסגור את היום', { duration: 12000 });
          }
        } catch (e) {
          console.warn('[Notifications] סיום יום:', e);
        }
      }
    }
    
  }, [tasks, user?.id, canNotify, canNotifyForTask, markNotified, markNotifiedForTask, sendNotification, appNotifications]);
