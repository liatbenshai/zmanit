...
function getUserWorkSettings(userId) {
  ...
  try {
    const userSettingsRaw = localStorage.getItem(`work_settings_${userId}`);
    if (userSettingsRaw) {
      const parsed = JSON.parse(userSettingsRaw);
      if (parsed?.workHours) {
        return {
          startHour: parsed.workHours.startHour ?? fallback.startHour,
          startMinute: parsed.workHours.startMinute ?? fallback.startMinute,
          endHour: parsed.workHours.endHour ?? fallback.endHour,
          endMinute: parsed.workHours.endMinute ?? fallback.endMinute,
          workDays: parsed.workHours.workDays ?? fallback.workDays
        };
      }
    }

    const generalSettingsRaw = localStorage.getItem('zmanit_work_settings');
    if (generalSettingsRaw) {
      const parsed = JSON.parse(generalSettingsRaw);
      const startMinutes = parsed.startMinutes ?? (fallback.startHour * 60 + fallback.startMinute);
      const endMinutes = parsed.endMinutes ?? (fallback.endHour * 60 + fallback.endMinute);
      return {
    ...
    console.warn('⚠️ שגיאה בקריאת הגדרות שעות עבודה:', e);
  }
...
export async function createTaskWithIntervals(task) {
  ...
  const userWorkSettings = getUserWorkSettings(task.user_id);
  const configuredWorkStart = userWorkSettings.startHour + (userWorkSettings.startMinute / 60);
  const configuredWorkEnd = userWorkSettings.endHour + (userWorkSettings.endMinute / 60);

  // משימות בית משובצות מחוץ לשעות העבודה (אחה"צ/ערב)
  const dayEndHour = isHomeTask ? 21 : configuredWorkEnd;
  const dayStartHour = isHomeTask ? Math.max(configuredWorkEnd, 16.5) : configuredWorkStart;
  ...
  if (currentDate > todayISO) {
    ...
    if (isHomeTask) {
      ...
      if (currentTime.hours >= dayEndHour) {
        currentDate = getNextWorkDay(currentDate, task.user_id);
        currentTime = {
          hours: userWorkSettings.startHour,
          minutes: userWorkSettings.startMinute
        };
      }
    }
...
function getNextWorkDay(dateStr, userId) {
  const settings = getUserWorkSettings(userId);
  const workDays = Array.isArray(settings.workDays) && settings.workDays.length > 0
    ? settings.workDays
    : [0, 1, 2, 3, 4];
  ...
  while (!workDays.includes(date.getDay())) {
    date.setDate(date.getDate() + 1);
  }
